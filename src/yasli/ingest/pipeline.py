...
# The frozen copy of ``LATEST_KEY`` committed for local seeding, so a
# developer needs no R2 credential. Regenerated by ``yasli.seed freeze``.
# Resolved off the package, not the process CWD.
DEFAULT_SNAPSHOT = REPO_ROOT / "data" / "seed" / "snapshot.json.gz"
...
class SnapshotFileError(ValueError):
    """Raised when a local snapshot path cannot be read as snapshot bytes.

    Covers missing, unreadable and corrupt-gzip files alike, so the CLI can
    report the path rather than leak an ``OSError`` or a ``gzip`` traceback.
    Sits beside :class:`UnsupportedSnapshotVersion` under the CLI's exit
    code 3 — both mean "the snapshot did not read", not "R2 was down".
    """
...
def _fetch_snapshot_bytes(
    client: Any | None, *, snapshot_path: Path | None = None
) -> bytes:
    """Phase 1: fetch the snapshot bytes.

    With ``snapshot_path`` the bytes come from that local file (a ``.gz`` is
    transparently decompressed) and R2 is never touched — that is what lets
    a developer seed without a credential. Without it, the R2 branch is
    unchanged: one ``get_object`` on :data:`LATEST_KEY`.

    Raises :class:`SnapshotFileError` when the path cannot be read.
    """
    ...
    try:
        ...
        raise SnapshotFileError(
            f"cannot read snapshot file {snapshot_path}: {exc}"
        ) from exc
    ...
    try:
        ...
        raise SnapshotFileError(
            f"snapshot file {snapshot_path} is not a readable gzip archive: {exc}"
        ) from exc
...
def run(
    *, r2_client: Any | None = None, snapshot_path: Path | None = None
) -> IngestSummary:
    """Execute the full ingest pipeline and return the summary.

    ``snapshot_path`` swaps the R2 fetch for a local file; everything after
    phase 1 — validation, planning, the upserts, the gated stamping — is
    identical either way, which is the point: a local seed exercises the
    real ingest rather than a parallel loading path that could drift from it.
    """
    started = time.monotonic()

    body = _fetch_snapshot_bytes(r2_client, snapshot_path=snapshot_path)
