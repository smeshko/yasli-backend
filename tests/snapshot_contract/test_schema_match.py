...
committed `schemas/snapshot.v2.schema.json`.

The fixture in `fixtures/snapshot.v2.schema.json` is a manually synced copy
of `yasli/scraper/schemas/snapshot.v2.schema.json`. When `schema_version`
...
FIXTURE_PATH = Path(__file__).parent / "fixtures" / "snapshot.v2.schema.json"
...
def test_vendored_models_match_canonical_scraper_schema() -> None:
    if not FIXTURE_PATH.exists():
        pytest.skip(
            ...
            "yasli/scraper/schemas/snapshot.v2.schema.json into "
    ...
    assert _canonical(actual) == _canonical(expected), (
        ...
        "snapshot.v2.schema.json. Update src/yasli/snapshot_contract/models.py "
