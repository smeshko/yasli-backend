...
Asserts the three v1 tables are registered on `Base.metadata` and that
...
from yasli.models import AddressEntry, Base, Institution, Street
from yasli.models.types import KIND_VALUES


def test_metadata_registers_three_tables() -> None:
...
def test_orm_classes_resolve() -> None:
    ...
    assert AddressEntry.__tablename__ == "address_entries"


def test_institution_kind_literal_matches_check_constraint() -> None:
    hints = typing.get_type_hints(Institution, include_extras=False)
